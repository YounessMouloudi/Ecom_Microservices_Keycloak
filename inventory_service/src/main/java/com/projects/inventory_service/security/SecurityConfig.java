package com.projects.inventory_service.security;
...
//              .authorizeHttpRequests(ar -> ar.requestMatchers("/api/products/**").hasAuthority("ADMIN"))
              ...
              /* hadi pour indiquer a oauth2 que on va récuperé le public key depuis keycloak qui est dans certs
                ...
                le public key */
//              .oauth2Login(log -> log.loginPage("/login").defaultSuccessUrl("/"))
              /* hadi ila bghiti tchanger default login de keyck w f keyck khass t7adad lih bila ghadi dir login
                 dialk w khass t autoriser had l'url de login f permitAll => hadi cheftha m3ah f autre video */
              .build();
    }
